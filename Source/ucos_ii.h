...
typedef struct os_event
{
	uint8_t OSEventType; //0
	uint8_t OSEventGrp; //1
	uint16_t wfill_2; //2
	uint16_t OSEventCnt; //4
	void* OSEventPtr; //8
	uint8_t OSEventTbl[8/*OS_EVENT_TBL_SIZE*/]; //12
	//0x14
} OS_EVENT;
...
typedef struct os_mem {                   /* MEMORY CONTROL BLOCK                                      */
    ...
    void   *OSMemFreeList; //4               /* Pointer to list of free memory blocks                     */
    INT32U  OSMemBlkSize;                 /* Size (in bytes) of each block of memory                   */
    INT32U  OSMemNBlks;                   /* Total number of blocks in this partition                  */
    INT32U  OSMemNFree; //16                  /* Number of memory blocks remaining in this partition       */
...
typedef struct os_q {                   /* QUEUE CONTROL BLOCK                                         */
    struct os_q   *OSQPtr; //0             /* Link to next queue control block in list of free blocks     */
    void         **OSQStart; //4           /* Pointer to start of queue data                              */
    void         **OSQEnd; //8             /* Pointer to end   of queue data                              */
    void         **OSQIn; //12              /* Pointer to where next message will be inserted  in   the Q  */
    void         **OSQOut; //16             /* Pointer to where next message will be extracted from the Q  */
    INT16U         OSQSize; //20            /* Size of queue (maximum number of entries)                   */
    INT16U         OSQEntries; //22         /* Current number of entries in the queue                      */
} OS_Q;
...
#if 1 //OS_EVENT_EN && (OS_MAX_EVENTS > 0)
...
OS_EXT  /*OS_TCB*/RTOS_tTCB           *OSTCBCur;                        /* Pointer to currently running TCB         */
#if 0
